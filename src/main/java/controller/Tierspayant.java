...
public class Tierspayant extends HttpServlet {
    ...
    private static final Logger LOG = Logger.getLogger(Tierspayant.class.getName());
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (request.getParameter("lg_TYPE_TIERS_PAYANT_ID") != null) {
                id = request.getParameter("lg_TYPE_TIERS_PAYANT_ID");

            }
            if (request.getParameter("cmb_TYPE_TIERS_PAYANT") != null) {
                id = request.getParameter("cmb_TYPE_TIERS_PAYANT");

            }
            if (request.getParameter("query") != null) {
                searchValue = request.getParameter("query");
            ...
            List<TTiersPayant> list = showAllOrOneTierspayant(searchValue, id, start, limit);
            int count = Tierspayant.this.showAllOrOneTierspayant(searchValue, id);
            ...
            for (TTiersPayant tTiersPayant : list) {
                ...
                json.put("caution", tTiersPayant.getCaution());
    ...
    private List<TTiersPayant> showAllOrOneTierspayant(String searchValue, String id, int start, int limit) {
        ...
        try {
            ...
            predicate = cb.and(predicate, cb.equal(root.get(TTiersPayant_.strSTATUT), Constant.STATUT_ENABLE));

            if (!"".equals(searchValue)) {
                predicate = cb.and(predicate,
                        cb.or(cb.like(j.get(TTypeTiersPayant_.strLIBELLETYPETIERSPAYANT), searchValue + "%"),
                                cb.like(root.get(TTiersPayant_.strNAME), searchValue + "%"),
                                cb.like(root.get(TTiersPayant_.strCODEORGANISME), searchValue + "%")));

            }
            if (!"".equals(id)) {
                predicate = cb.and(predicate, cb.equal(j.get(TTypeTiersPayant_.lgTYPETIERSPAYANTID), id));
            }
            cq.where(predicate);
    ...
    private int showAllOrOneTierspayant(String searchValue, String id) {
