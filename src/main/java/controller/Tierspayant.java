    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            for (TTiersPayant tTiersPayant : list) {
                ...
                json.put("lgGROUPEID", (tTiersPayant.getLgGROUPEID() != null) ? tTiersPayant.getLgGROUPEID().getStrLIBELLE() : "");

                json.put("lg_CUSTOMER_ID", tTiersPayant.getLgTIERSPAYANTID());
                json.put("str_LIBELLE", tTiersPayant.getStrNAME());
                json.put("lg_VILLE_ID", (tTiersPayant.getLgVILLEID() != null ? tTiersPayant.getLgVILLEID().getStrName() : ""));
                json.put("lg_TYPE_TIERS_PAYANT_ID", (tTiersPayant.getLgTYPETIERSPAYANTID() != null ? tTiersPayant.getLgTYPETIERSPAYANTID().getStrLIBELLETYPETIERSPAYANT() : ""));
                json.put("lg_TYPE_CONTRAT_ID", (tTiersPayant.getLgTYPECONTRATID() != null ? tTiersPayant.getLgTYPECONTRATID().getStrLIBELLETYPECONTRAT() : ""));
                json.put("lg_RISQUE_ID", (tTiersPayant.getLgRISQUEID() != null ? tTiersPayant.getLgRISQUEID().getStrLIBELLERISQUE() : ""));
                ...
                json.put("dbl_PLAFOND_CREDIT", tTiersPayant.getDblPLAFONDCREDIT());
                json.put("nbrbons", (tTiersPayant.getIntNBREBONS() != null) ? (tTiersPayant.getIntNBREBONS() > 0 ? tTiersPayant.getIntNBREBONS() : 0) : 0);
                json.put("montantFact", (tTiersPayant.getIntMONTANTFAC() != null) ? (tTiersPayant.getIntMONTANTFAC() > 0 ? tTiersPayant.getIntMONTANTFAC() : 0) : 0);
...
//            root.fetch("tCompteClientTiersPayantCollection", JoinType.INNER);
            cq.select(root)/*.distinct(true).groupBy(root.get(TTiersPayant_.lgTIERSPAYANTID))*/.orderBy(cb.asc(j.get(TTypeTiersPayant_.strLIBELLETYPETIERSPAYANT)), cb.asc(root.get(TTiersPayant_.strNAME)));
            Predicate predicate = cb.conjunction();
            predicate = cb.and(predicate, cb.equal(root.get(TTiersPayant_.strSTATUT), "enable"));

            if (!"".equals(search_value)) {
                predicate = cb.and(predicate, cb.or(cb.like(j.get(TTypeTiersPayant_.strLIBELLETYPETIERSPAYANT), search_value + "%"), cb.like(root.get(TTiersPayant_.strNAME), search_value + "%"), cb.like(root.get(TTiersPayant_.strCODEORGANISME), search_value + "%")));

            }
            if (!"".equals(lg_TYPE_TIERS_PAYANT_ID)) {
                predicate = cb.and(predicate, cb.equal(j.get(TTypeTiersPayant_.lgTYPETIERSPAYANTID), lg_TYPE_TIERS_PAYANT_ID));
            }
            cq.where(predicate);
    ...
    private TCompteClient getTCompteClient(String P_KEY) {
        ...
        try {
            OTCompteClient = em.createQuery("SELECT t FROM TCompteClient t WHERE t.pKey = ?1 AND t.strSTATUT = ?2", TCompteClient.class).
                    ...
                    setParameter(2, commonparameter.statut_enable).setMaxResults(1).
                    getSingleResult();
