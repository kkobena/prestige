...
public class Deconditionnement extends HttpServlet {
    ...
    private TFamilleStock getTProductItemStock(String lg_FAMILLE_ID, String lg_EMPLACEMENT_ID, EntityManager em) {
        ...
        TFamilleStock OTProductItemStock = (TFamilleStock) em.
    ...
    private boolean doDeconditionnement(String lg_FAMILLE_ID, int int_NUMBER, TUser user, dataManager manager) {
        ...
        try {
            ...
            TFamilleStock OTFamilleStockParent = getTProductItemStock(OTFamilleParent.getLgFAMILLEID(), OTEmplacement.getLgEMPLACEMENTID(), em);
            TFamilleStock OTFamilleStockChild = getTProductItemStock(OTFamilleChild.getLgFAMILLEID(), OTEmplacement.getLgEMPLACEMENTID(), em);
...
//                LOGGER.log(Level.INFO, "numberToDecondition --------------------------------------- {0}  {1}", new Object[]{numberToDecondition, x});
            ...
            String desc = "Déconditionnement du produit " + OTFamilleParent.getIntCIP() + " " + OTFamilleParent.getStrNAME() + " nombre de boîtes = " + numberToDecondition + " nombre de détails =" + (numberToDecondition * qtyDetail) + " opérateur " + user.getStrFIRSTNAME() + " " + user.getStrLASTNAME();
...
//                LOGGER.log(Level.ALL, "---------------------- mouvement stock -------------->>>", e);
            ...
            try {
                ...
                createSnapshotMouvementArticle(OTFamilleChild, OTFamilleStockChild.getIntNUMBERAVAILABLE(), stockInitDetail, OTEmplacement, em);
            }
            try {
                TMouvementSnapshot mouvementSnapshot = findMouvementSnapshotByDay(OTFamilleParent, OTEmplacement.getLgEMPLACEMENTID(), em);
...
//                LOGGER.log(Level.ALL, "---------------------- mouvementSnapshot -------------->>>", e);
            ...
            if (OTEmplacement.getLgEMPLACEMENTID().equalsIgnoreCase(commonparameter.PROCESS_SUCCESS) && OTFamilleParent.getBCODEINDICATEUR() == 0) {
    ...
    private TMouvement findByDay(TFamille OTFamille, String lgEmpl, EntityManager em) throws Exception {
        TypedQuery<TMouvement> query = em.createQuery("SELECT o FROM TMouvement o  WHERE o.dtDAY =?1 AND o.lgFAMILLEID.lgFAMILLEID =?2 AND o.lgEMPLACEMENTID.lgEMPLACEMENTID=?3 AND o.strACTION =?4", TMouvement.class);
    ...
    public void createTMouvement(TFamille OTFamille, TEmplacement OTEmplacement, String str_TYPE_ACTION, String str_ACTION, Integer int_NUMBER, TUser user, EntityManager em) throws Exception {
        date Odate = new date();
    ...
    private TMouvementSnapshot findMouvementSnapshotByDay(TFamille OTFamille, String lgEmpl, EntityManager em) throws Exception {
        TypedQuery<TMouvementSnapshot> query = em.createQuery("SELECT o FROM TMouvementSnapshot o  WHERE o.dtDAY =?1 AND o.lgFAMILLEID.lgFAMILLEID =?2 AND o.lgEMPLACEMENTID.lgEMPLACEMENTID=?3 AND o.strSTATUT='enable' ", TMouvementSnapshot.class);
        query.setParameter(1, new Date(), TemporalType.DATE);
        query.setParameter(2, OTFamille.getLgFAMILLEID());
        query.setParameter(3, lgEmpl);
        query.setFirstResult(0).setMaxResults(1);
    ...
    public void createSnapshotMouvementArticle(TFamille OTFamille, int int_NUMBER, int int_STOCK_DEBUT, TEmplacement OTEmplacement, EntityManager em) throws Exception {
        date Odate = new date();
    ...
    public void updateSnapshotMouvementArticle(TMouvementSnapshot OTMouvementSnapshot, int int_NUMBER, EntityManager em) throws Exception {
        OTMouvementSnapshot.setDtUPDATED(new Date());
    ...
    private boolean doDeconditionnementStock(String lg_FAMILLE_ID, int int_NUMBER, TUser user, dataManager manager) {
        ...
        try {
            ...
            OTFamilleStockChild.setIntNUMBERAVAILABLE(OTFamilleStockChild.getIntNUMBERAVAILABLE() + (int_NUMBER * qtyDetail));
            OTFamilleStockChild.setIntNUMBER(OTFamilleStockChild.getIntNUMBERAVAILABLE());
            OTFamilleStockChild.setDtUPDATED(new Date());
            em.getTransaction().begin();
            em.merge(OTFamilleStockParent);
            ...
            mouvementProduitService.saveMvtProduit(p.getLgDECONDITIONNEMENTID(),
                    DateConverter.DECONDTIONNEMENT_NEGATIF, OTFamilleParent, user,
                    OTEmplacement, int_NUMBER, stockInit, stockInit - int_NUMBER, em, 0);
            mouvementProduitService.saveMvtProduit(child.getLgDECONDITIONNEMENTID(),
                    DateConverter.DECONDTIONNEMENT_POSITIF, OTFamilleChild, user,
            ...
            try {
                ...
                createTMouvement(OTFamilleChild, OTEmplacement, commonparameter.ADD, commonparameter.str_ACTION_DECONDITIONNEMENT, (int_NUMBER * qtyDetail), user, em);
                createTMouvement(OTFamilleParent, OTEmplacement, commonparameter.REMOVE, commonparameter.str_ACTION_DECONDITIONNEMENT, int_NUMBER, user, em);
