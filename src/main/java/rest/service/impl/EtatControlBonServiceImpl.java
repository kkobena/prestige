...
public class EtatControlBonServiceImpl implements EtatControlBonService {
    ...
    @Override
    public List<EtatControlBon> list(boolean fullAuth, String search, String dtStart, String dtEnd, String grossisteId, int start, int limit, boolean all) {
    ...
    @Override
    public JSONObject list(boolean fullAuth, String search, String dtStart, String dtEnd, String grossisteId, int start, int limit) {
        long count = count(search, dtStart, dtEnd, grossisteId);
        return FunctionUtils.returnData(list(fullAuth, search, dtStart, dtEnd, grossisteId, start, limit, false), count);
    }
    
    @Override
    public EtatControlAnnuelWrapperDTO listBonAnnuel(String groupBy, String dtStart, String dtEnd, String grossisteId, Integer groupeId) {
        ...
        annuelSummary.setEtatControlAnnuels(list.stream().peek(el
                -> el.setPourcentage(computePercent(el.getMontantTtc(), annuelSummary.getSummary().getTotalTtc())))
                .sorted(Comparator.comparing(EtatControlAnnuelDTO::getGroupByLibelle))
                .collect(Collectors.toList()));
    ...
    private float computePercent(long montantTTC, long totalTTC) {
        return BigDecimal.valueOf((montantTTC * 100) / Double.valueOf(totalTTC)).setScale(2, RoundingMode.HALF_UP).floatValue();
    }
    ...
    private EtatControlAnnuelDTO buildEtatControlAnnuels(Tuple tuple) {
        ...
        return EtatControlAnnuelDTO.builder()
                .montantMarge(marge)
                .montantHtaxe(tuple.get("montantHtaxe", BigDecimal.class).longValue())
                .nbreBon(tuple.get("nbreBon", BigInteger.class).intValue())
                .montantTtc(montantTtc)
                .montantTaxe(montantTaxe)
                .groupByLibelle(tuple.get("groupByLibelle", String.class))
                .montantVenteTtc(montantVenteTtc)
                .build();
    }
    
    private List<EtatControlAnnuelDTO> listBonsAnnuel(String groupBy, String dtStart, String dtEnd, String grossisteId, Integer groupeId) {
    ...
    private EtatControlAnnuelWrapperDTO bonsAnnuelGroup(String dtStart, String dtEnd, String grossisteId, Integer groupeId) {
    ...
    private EtatControlAnnuelWrapperDTO buildEtatControlAnnuelSummaryDTO(Tuple tuple) {
        ...
        return EtatControlAnnuelWrapperDTO.builder()
                .summary(EtatControlAnnuelWrapperDTO.EtatControlAnnuelSummary.builder()
                        .totalMarge(marge)
                        .totaltHtaxe(tuple.get("montantHtaxe", BigDecimal.class).longValue())
                        .totalNbreBon(tuple.get("nbreBon", BigInteger.class).intValue())
                        .totalTtc(montantTtc)
                        .totalTaxe(montantTaxe)
                        .totalVenteTtc(montantVenteTtc)
                        .build())
                .build();
    }
    
    @Override
    public JSONObject listBonAnnuelView(String groupBy, String dtStart, String dtEnd, String grossisteId, Integer groupeId) {
