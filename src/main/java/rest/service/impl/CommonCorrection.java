...
public class CommonCorrection {
    ...
    private List<Item> fetch() {
        ...
        try {
            Query q = entityManager.createNativeQuery("SELECT t.cip,t.nom,t.tab,t.tva,f.lg_FAMILLE_ID AS id,f.int_PRICE AS price,f.int_T AS code FROM t_famille f, tva t WHERE f.int_CIP=t.cip", Tuple.class);
            List<Tuple> l = q.getResultList();
            l.forEach((t) -> {
                Item item = new Item(t.get(4, String.class), t.get(0, String.class), t.get(1, String.class), Integer.valueOf(t.get(3, String.class)), t.get(2, String.class), t.get(5, Integer.class), t.get(6, String.class));
                items.add(item);
            });
    ...
    public int updateVente() {
        ...
        findPreenregistrement().forEach((p) -> {
            ...
            int montantTva = 0;
            List<TPreenregistrementDetail> details = findPreenregistrementDetailsByIdVente(p.getLgPREENREGISTREMENTID());
    ...
    List<TPreenregistrementDetail> findPreenregistrementDetails(String id) {
        TypedQuery<TPreenregistrementDetail> query = entityManager.createQuery("SELECT o FROM TPreenregistrementDetail o WHERE o.lgFAMILLEID.lgFAMILLEID=?1 AND FUNCTION('DATE',o.dtCREATED) BETWEEN  ?2 AND ?3", TPreenregistrementDetail.class);
    ...
    List<HMvtProduit> findHMvtProduits(String id) {
        TypedQuery<HMvtProduit> query = entityManager.createQuery("SELECT o FROM HMvtProduit o WHERE o.famille.lgFAMILLEID = ?1  AND o.mvtDate BETWEEN ?2 AND ?3", HMvtProduit.class);
        query.setParameter(1, id);
    ...
    List<TFamilleGrossiste> findHFamilleGrossistes(String id) {
        TypedQuery<TFamilleGrossiste> query = entityManager.createQuery("SELECT o FROM TFamilleGrossiste o WHERE o.lgFAMILLEID.lgFAMILLEID = ?1", TFamilleGrossiste.class);
        query.setParameter(1, id);
        return query.getResultList();
    }

    List<TPreenregistrement> findPreenregistrement() {
        TypedQuery<TPreenregistrement> query = entityManager.createQuery("SELECT o FROM TPreenregistrement o WHERE  FUNCTION('DATE',o.dtCREATED) BETWEEN  ?1 AND ?2", TPreenregistrement.class);
    ...
    List<MvtTransaction> findMvtTransactions(String id) {
        TypedQuery<MvtTransaction> query = entityManager.createQuery("SELECT o FROM MvtTransaction o WHERE o.pkey = ?1  ", MvtTransaction.class);
        query.setParameter(1, id);
        return query.getResultList();
    }

    List<TPreenregistrementDetail> findPreenregistrementDetailsByIdVente(String id) {
        TypedQuery<TPreenregistrementDetail> query = entityManager.createQuery("SELECT o FROM TPreenregistrementDetail o WHERE o.lgPREENREGISTREMENTID.lgPREENREGISTREMENTID =?1 ", TPreenregistrementDetail.class);
        query.setParameter(1, id);
        return query.getResultList();
    }
