...
//recupere l'imprimante
    public TImprimante getTImprimante(String lg_IMPRIMANTE_ID) {
        ...
        try {
            OTImprimante = (TImprimante) this.getOdataManager().getEm().createQuery("SELECT t FROM TImprimante t WHERE (t.lgIMPRIMANTEID LIKE ?1 OR t.strNAME LIKE ?1) AND t.strSTATUT = ?2")
    ...
    public List<TImprimante> getListeImprimante(String search_value, String lg_IMPRIMANTE_ID) {
        ...
        try {
            ...
            lstTImprimante = this.getOdataManager().getEm().createQuery("SELECT t FROM TImprimante t WHERE (t.lgIMPRIMANTEID LIKE ?1 OR t.strNAME LIKE ?1) AND t.strSTATUT = ?2 AND (t.strDESCRIPTION LIKE ?3 OR t.strNAME LIKE ?3) ORDER BY t.strDESCRIPTION ASC")
                    .setParameter(1, lg_IMPRIMANTE_ID).setParameter(2, commonparameter.statut_enable).setParameter(3, search_value+"%").getResultList();
        } catch (Exception e) {
            // e.printStackTrace();
        }
...
/*
    public List<TUserImprimante> getListeImprimanteByUser(String lg_USER_ID, String lg_IMPRIMANTE_ID) {
        ...
        try {
            lstTUserImprimante = this.getOdataManager().getEm().createQuery("SELECT t FROM TUserImprimante t WHERE t.lgIMPRIMANTEID.lgIMPRIMANTEID LIKE ?1 AND t.lgUSERID.lgUSERID LIKE ?2 AND t.strSTATUT = ?3 ORDER BY t.dtCREATED DESC")
                    .setParameter(1, lg_IMPRIMANTE_ID).setParameter(2, lg_USER_ID).setParameter(3, commonparameter.statut_enable).getResultList();
        } catch (Exception e) {
            // e.printStackTrace();
        }
        new logger().OCategory.info("Taille liste " + lstTUserImprimante.size());
        return lstTUserImprimante;
    }
    //fin Liste des imprimantes

    //recuperation de l'imprimante d'un utilisateur
    public TUserImprimante getTUserImprimante(String lg_USER_ID, String lg_IMPRIMANTE_ID, String str_NAME) {
        TUserImprimante OTUserImprimante = null;
        List<TUserImprimante> lstTUserImprimante = new ArrayList<TUserImprimante>();
        try {
            lstTUserImprimante = this.getOdataManager().getEm().createQuery("SELECT t FROM TUserImprimante t WHERE t.lgUSERID.lgUSERID = ?1 AND t.lgIMPRIMANTEID.lgIMPRIMANTEID LIKE ?2 AND t.strNAME = ?3 AND t.strSTATUT = ?4")
                    .setParameter(1, lg_USER_ID).setParameter(2, lg_IMPRIMANTE_ID).setParameter(3, str_NAME).setParameter(4, commonparameter.statut_enable).getResultList();
            if(lstTUserImprimante.size() > 0) {
                OTUserImprimante = lstTUserImprimante.get(0);
                this.buildSuccesTraceMessage("Utilisateur valide");
            } else {
        ...
        return OTUserImprimante;
    }
    
    public TUserImprimante getTUserImprimante(String lg_USER_ID, String lg_IMPRIMANTE_ID) {
        TUserImprimante OTUserImprimante = null;
        try {
            OTUserImprimante = (TUserImprimante) this.getOdataManager().getEm().createQuery("SELECT t FROM TUserImprimante t WHERE t.lgUSERID.lgUSERID = ?1 AND t.lgIMPRIMANTEID.lgIMPRIMANTEID LIKE ?2 AND t.strSTATUT = ?4")
                    .setParameter(1, lg_USER_ID).setParameter(2, lg_IMPRIMANTE_ID).setParameter(4, commonparameter.statut_enable).getSingleResult();
            this.buildSuccesTraceMessage("Utilisateur valide");
        } catch (Exception e) {
            e.printStackTrace();
            this.buildErrorTraceMessage("Utilisateur non abilité à utiliser l'imprimante");
    ...
    //assignation d'une imprimante a un utilisateur
    public boolean createUserImprimante(TImprimante OTImprimante, TUser OTUser, String str_NAME) {
        boolean result = false;
        try {
            ...
            OTUserImprimante.setStrNAME(str_NAME);
            OTUserImprimante.setStrDESCRIPTION(OTUserImprimante.getStrNAME());
            ...
            e.printStackTrace();
            this.buildErrorTraceMessage("Echec d'assignation de l'imprimante à l'utilisateur sélectionné");
        }
        return result;
    }
    ...
    //suppression d'une imprimante a un utilisateur
    public boolean deleteUserImprimante(String lg_USER_IMPRIMQNTE_ID) {
        boolean result = false;
        try {
            TUserImprimante OTUserImprimante = this.getOdataManager().getEm().find(TUserImprimante.class, lg_USER_IMPRIMQNTE_ID);
            this.delete(OTUserImprimante);
            result = true;
            this.buildSuccesTraceMessage(this.getOTranslate().getValue("SUCCES"));
        } catch (Exception e) {
            e.printStackTrace();
            this.buildErrorTraceMessage("Echec de suppression de l'imprimante pour cet utilisateur sélectionné");
        }
        return result;
    }
    ...
    */
    public List<EntityData> showAllOrOneUserByImprimante(String search_value, String lg_USER_ID, String lg_IMPRIMANTE_ID) {
...
//                OEntityData.setStr_value3(String.valueOf(this.isExistUserImprimante(lg_USER_ID, OTImprimante.getLgIMPRIMANTEID())));
     ...
     /*
    public boolean isExistUserImprimante(String lg_USER_ID, String lg_IMPRIMANTE_ID) {
        boolean result = false;
        try {
