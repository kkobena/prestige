...
public class grossisteManagement extends bllBase {
    ...
    public void create(String str_LIBELLE, String str_DESCRIPTION, String str_ADRESSE_RUE_1,
            String str_ADRESSE_RUE_2, String str_CODE_POSTAL, String str_BUREAU_DISTRIBUTEUR,
            String str_MOBILE, String str_TELEPHONE, int int_DELAI_REGLEMENT_AUTORISE,
            String lg_TYPE_REGLEMENT_ID, String lg_VILLE_ID, Double dbl_CHIFFRE_DAFFAIRE, String str_CODE,
            int int_DELAI_REAPPROVISIONNEMENT, int int_COEF_SECURITY, int int_DATE_BUTOIR_ARTICLE, String groupeId, String idrepartiteur) {
    ...
    public void update(String lg_GROSSISTE_ID, String str_LIBELLE, String str_DESCRIPTION, String str_ADRESSE_RUE_1,
            String str_ADRESSE_RUE_2, String str_CODE_POSTAL, String str_BUREAU_DISTRIBUTEUR,
            String str_MOBILE, String str_TELEPHONE, int int_DELAI_REGLEMENT_AUTORISE,
            String lg_TYPE_REGLEMENT_ID, String lg_VILLE_ID, String str_CODE,
            int int_DELAI_REAPPROVISIONNEMENT, int int_COEF_SECURITY, int int_DATE_BUTOIR_ARTICLE, String groupeId, String idrepartiteur) {
        ...
        try {
            ...
            dal.TTypeReglement OTTypeReglement = getOdataManager().getEm().find(dal.TTypeReglement.class, lg_TYPE_REGLEMENT_ID);
            if (OTTypeReglement != null) {
                OTGrossiste.setLgTYPEREGLEMENTID(OTTypeReglement);
            }
    ...
    public TGrossiste getGrossiste(String lg_GROSSISTE_ID) {
        ...
        try {
            OTGrossiste = (TGrossiste) this.getOdataManager().getEm().createQuery("SELECT t FROM TGrossiste t WHERE (t.lgGROSSISTEID = ?1 OR t.strLIBELLE = ?1 OR t.strCODE = ?1) AND t.strSTATUT = ?2")
    ...
    public boolean deleteGrossiste(String lg_GROSSISTE_ID) {
        ...
        try {
            ...
            if (this.delete(OGrossiste)) {
                ...
                this.buildErrorTraceMessage("Impossible de supprimer un grossiste qui a déjà subit une transaction dans le système");
            }
        } catch (Exception e) {
    ...
    public List<TGrossiste> getListeGrossiste(String search_value, String lg_GROSSISTE_ID) {
        ...
        try {
            ...
            lstTGrossiste = this.getOdataManager().getEm().createQuery("SELECT t FROM TGrossiste t WHERE t.lgGROSSISTEID LIKE ?1 AND (t.strLIBELLE LIKE ?2 OR t.strCODE LIKE ?2) AND t.strSTATUT LIKE ?3 ORDER BY t.strDESCRIPTION ASC").
    ...
    public boolean isGrossisteExist(String str_CODE) {
        ...
        try {
            ...
            OTGrossiste = (TGrossiste) this.getOdataManager().getEm().createQuery("SELECT t FROM TGrossiste t WHERE t.strCODE = ?1 AND t.strSTATUT LIKE ?2").
                    setParameter(1, str_CODE)
                    .setParameter(2, commonparameter.statut_enable)
                    .getSingleResult();
            if (OTGrossiste != null) {
                this.buildErrorTraceMessage("Désolé. Le grossiste " + OTGrossiste.getStrDESCRIPTION() + " a déjà le code " + str_CODE);
                result = true;
            }
        } catch (Exception e) {
//            e.printStackTrace();
    ...
    public List<TGrossiste> getListGrossisteFromInventaire(String search_value, String lg_INVENTAIRE_ID) {
        ...
        try {
            ...
            String qry = "SELECT DISTINCT(t.lg_GROSSISTE_ID), t.str_LIBELLE FROM t_grossiste t, t_inventaire_famille i, t_famille f WHERE f.lg_GROSSISTE_ID = t.lg_GROSSISTE_ID AND f.lg_FAMILLE_ID = i.lg_FAMILLE_ID AND (t.str_LIBELLE LIKE '" + search_value + "%' OR t.str_CODE LIKE '" + search_value + "%' OR t.str_DESCRIPTION LIKE '" + search_value + "%') AND i.lg_INVENTAIRE_ID = '" + lg_INVENTAIRE_ID + "' ORDER BY t.str_LIBELLE";
