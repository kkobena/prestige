...
public class dciManagement extends bllBase {
    ...
    public void create(String STR_CODE, String STR_NAME) {
        ...
        try {
            ...
            TDci OTDci = new TDci();
            OTDci.setLgDCIID(this.getKey().getComplexId()); // Génération automatique d'un ID à partir de la date courante
    ...
    public List<TDci> showAllInitial(String search_value) {
        ...
        try {
            ...
            lstTDci = this.getOdataManager().getEm().createQuery("SELECT t FROM TDci t WHERE t.strSTATUT = ?1 AND t.strNAME LIKE ?2")
    ...
    public TDci getDci(String lg_DCI_ID) {
        TDci OTDci = null;
        try {
            OTDci = (TDci) this.getOdataManager().getEm().createQuery("SELECT t FROM TDci t WHERE t.strSTATUT = ?1 AND (t.lgDCIID = ?2 OR t.strCODE = ?2 OR t.strNAME = ?2)")
    ...
    public TFamilleDci getFamilleDci(String lg_FAMILLE_ID, String lg_DCI_ID) {
        ...
        try {
            OTFamilleDci = (TFamilleDci) this.getOdataManager().getEm().createQuery("SELECT t FROM TFamilleDci t WHERE (t.lgDCIID.lgDCIID = ?2 OR t.lgDCIID.strNAME = ?2) AND t.lgFAMILLEID.lgFAMILLEID = ?1")
    ...
    public List<EntityData> ListDciFamille(String search_value, String lg_FAMILLE_ID, String lg_DCI_ID) {
        ...
        try {
            ...
            String qry = "SELECT * FROM v_famille_dci v WHERE v.lg_FAMILLE_ID LIKE '" + lg_FAMILLE_ID + "' AND v.lg_DCI_ID LIKE '" + lg_DCI_ID + "' AND (v.str_DESCRIPTION LIKE '" + search_value + "%' OR v.int_CIP LIKE '" + search_value + "%' OR v.int_EAN13 LIKE '" + search_value + "%' OR v.dci_str_NAME LIKE '" + search_value + "%' OR v.str_CODE LIKE '" + search_value + "%') AND v.str_STATUT = '" + commonparameter.statut_enable + "' ";
