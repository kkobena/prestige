...
public class RetrocessionDetailManagement extends bllBase {
    ...
    public boolean createRetrocessionDetail(int int_Qte_facture, boolean bool_T_F, String lg_FAMILLE_ID, TRetrocession OTRetrocession, int int_REMISE) {
        ...
        try {
            ...
            int_PRICE_OLD_TTC = (OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) + (((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) * OTRetrocessionDetail.getLgFAMILLEID().getLgCODETVAID().getIntVALUE()) / 100);
            OTRetrocessionDetail.setIntPRICE(bool_T_F ? OTFamille.getIntPAF() : (OTFamille.getIntPAT() - ((int_REMISE * OTFamille.getIntPAT()) / 100)));
            ...
            OTRetrocession.setIntMONTANTHT(OTRetrocession.getIntMONTANTHT() + ((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) - int_PRICE_OLD));
            OTRetrocession.setIntMONTANTTTC(OTRetrocession.getIntMONTANTTTC() + ((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) + (((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) * OTRetrocessionDetail.getLgFAMILLEID().getLgCODETVAID().getIntVALUE()) / 100)) - int_PRICE_OLD_TTC);
    ...
    public TRetrocessionDetail createRetrocessionDetail(TFamille OTFamille, TRetrocession OTRetrocession, boolean bool_T_F) {
    ...
    public TRetrocessionDetail updateRetrocessionDetail(String lg_RETROCESSIONDETAIL_ID, int int_Qte_facture, boolean bool_T_F, int int_PRICE, int int_REMISE) {
        TRetrocession OTRetrocession = null;
        int int_PRICE_OLD = 0, int_PRICE_OLD_TTC = 0;
        try {
            OTRetrocessionDetail = this.getOdataManager().getEm().find(TRetrocessionDetail.class, lg_RETROCESSIONDETAIL_ID);
            if(OTRetrocessionDetail == null) {
            ...
            OTRetrocessionDetail.setIntPRICE(bool_T_F ? OTRetrocessionDetail.getLgFAMILLEID().getIntPAF() : (OTRetrocessionDetail.getLgFAMILLEID().getIntPAT() - ((int_REMISE * OTRetrocessionDetail.getLgFAMILLEID().getIntPAT()) / 100)));
    ...
    public TRetrocession removeRetrocessionDetail(String lg_RETROCESSIONDETAIL_ID) {
        ...
        try {
            OTRetrocessionDetail = this.getOdataManager().getEm().find(TRetrocessionDetail.class, lg_RETROCESSIONDETAIL_ID);
            if (OTRetrocessionDetail == null) {
                this.buildErrorTraceMessage("Impossible de supprimer le produit dans cette retrocession. Produit inexistant");
                return OTRetrocession;
            }
            OTRetrocession = OTRetrocessionDetail.getLgRETROCESSIONID();
            OTRetrocession.setIntMONTANTHT(OTRetrocession.getIntMONTANTHT() - (OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()));
            OTRetrocession.setIntMONTANTTTC(OTRetrocession.getIntMONTANTTTC() - ((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) + (((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getIntQtefacture()) * OTRetrocessionDetail.getLgFAMILLEID().getLgCODETVAID().getIntVALUE()) / 100)));
            OTRetrocession.setDtUPDATED(new Date());
    ...
    public List<TRetrocessionDetail> SearchAllOrOneDetailRetrocession(String lg_RETROCESSIONDETAIL_ID) {
        ...
        try {
            ...
            lstTRetrocessionDetail = this.getOdataManager().getEm().createQuery("SELECT t FROM TRetrocessionDetail t WHERE t.lgRETROCESSIONID.lgRETROCESSIONID = ?1 AND t.strSTATUT = ?2")
                    .setParameter(1, lg_RETROCESSIONDETAIL_ID).setParameter(2, commonparameter.statut_enable).getResultList();
            for (int i = 0; i < lstTRetrocessionDetail.size(); i++) {
                new logger().OCategory.info("Retrocession: " + lstTRetrocessionDetail.get(i).getLgRETROCESSIONID().getStrREFERENCE());
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public List<TRetrocessionDetail> showOneOrAllRetrocessionDetailByRetrocession(String search_value, String lg_RETROCESSION_ID, String str_STATUT) {
        List<TRetrocessionDetail> lstTRetrocessionDetail = new ArrayList<TRetrocessionDetail>();
        try {
            lstTRetrocessionDetail = this.getOdataManager().getEm().createQuery("SELECT t FROM TRetrocessionDetail t WHERE t.lgRETROCESSIONID.lgRETROCESSIONID = ?1 AND t.strSTATUT = ?2 AND (t.lgFAMILLEID.intCIP LIKE ?3 OR t.lgFAMILLEID.strDESCRIPTION LIKE ?3 OR t.lgFAMILLEID.intEAN13 LIKE ?3)")
                    .setParameter(1, lg_RETROCESSION_ID).setParameter(2, str_STATUT).setParameter(3, search_value + "%").getResultList();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return lstTRetrocessionDetail;
    ...
    public List<TRetrocessionDetail> showOneOrAllRetrocessionDetailByRetrocession(String search_value, String lg_RETROCESSION_ID, String str_STATUT, int start, int limit) {
        List<TRetrocessionDetail> lstTRetrocessionDetail = new ArrayList<TRetrocessionDetail>();
        try {
            lstTRetrocessionDetail = this.getOdataManager().getEm().createQuery("SELECT t FROM TRetrocessionDetail t WHERE t.lgRETROCESSIONID.lgRETROCESSIONID = ?1 AND t.strSTATUT = ?2 AND (t.lgFAMILLEID.intCIP LIKE ?3 OR t.lgFAMILLEID.strDESCRIPTION LIKE ?3 OR t.lgFAMILLEID.intEAN13 LIKE ?3)")
                    .setParameter(1, lg_RETROCESSION_ID).setParameter(2, str_STATUT).setParameter(3, search_value + "%").setFirstResult(start).setMaxResults(limit).getResultList();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return lstTRetrocessionDetail;
    ...
    public int GetVenteTTC(String lg_RETROCESSION_ID) {
        ...
        try {
            ...
            lstTRetrocessionDetail = this.showOneOrAllRetrocessionDetailByRetrocession(lg_RETROCESSION_ID);
            for (TRetrocessionDetail OTRetrocessionDetail : lstTRetrocessionDetail) {
                Total_vente_ttc += (OTRetrocessionDetail.getIntPRICE() + ((OTRetrocessionDetail.getIntPRICE() * OTRetrocessionDetail.getLgFAMILLEID().getLgCODETVAID().getIntVALUE()) / 100)) * OTRetrocessionDetail.getIntQtefacture();
            }
        } catch (Exception e) {
            e.printStackTrace();
            new logger().OCategory.info("Retrocession inexistante");
    ...
    public boolean isStockAvailable(String lg_FAMILLE_ID, int int_QUANTITY) {
        ...
        try {
            ...
            //   new logger().OCategory.info("Famille " + OTFamilleStock.getLgFAMILLEID().getStrNAME() + " Stock " + OTFamilleStock.getIntNUMBERAVAILABLE());
            if (OTFamilleStock.getIntNUMBERAVAILABLE() >= int_QUANTITY) {
                result = true;
                new logger().OCategory.info("Stock disponible");
            } else {
                this.buildErrorTraceMessage("Stock de " + OTFamilleStock.getLgFAMILLEID().getStrNAME() + " insuffisant");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public TRetrocessionDetail isFamilleExist(String lg_FAMILLE_ID, String lg_RETROCESSION_ID) {
        TRetrocessionDetail OTRetrocessionDetail = null;
        try {
            OTRetrocessionDetail = (TRetrocessionDetail) this.getOdataManager().getEm().createQuery("SELECT t FROM TRetrocessionDetail t WHERE t.lgFAMILLEID.lgFAMILLEID = ?1 AND t.lgRETROCESSIONID.lgRETROCESSIONID = ?2 AND t.strSTATUT = ?3")
                    .setParameter(1, lg_FAMILLE_ID).setParameter(2, lg_RETROCESSION_ID).setParameter(3, commonparameter.statut_enable).getSingleResult();
        } catch (Exception e) {
            e.printStackTrace();
