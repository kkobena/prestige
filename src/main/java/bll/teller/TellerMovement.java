...
public class TellerMovement extends bll.bllBase {
    ...
    //03092019
    public TMvtCaisse AddTMvtCaisseDiff(TTypeMvtCaisse OTTypeMvtCaisse, String str_NUM_COMPTE, String str_NUM_PIECE_COMPTABLE, TReglement reglement, double int_AMOUNT, String str_BANQUE, String str_LIEU, String str_CODE_MONNAIE, String str_COMMENTAIRE_REGLEMENT, int int_TAUX, Date dt_DATE_MVT, boolean action, String str_FIRST_LAST_NAME, String P_KEY, String str_COMMENTAIRE, int int_AMOUNT_REMIS, int int_AMOUNT_RECU, boolean bool_CHECKED, Date dt_CREATED) {
...
//        OTReglement = new Preenregistrement(this.getOdataManager(), this.getOTUser()).CreateTReglement(this.getOTUser().getLgUSERID(), OTMvtCaisse.getLgMVTCAISSEID(), str_BANQUE, str_LIEU, str_CODE_MONNAIE, str_COMMENTAIRE_REGLEMENT, lg_MODE_REGLEMENT_ID, int_TAUX, int_AMOUNT,str_FIRST_LAST_NAME,new Date(), bool_CHECKED); // a decommenter en cas de probleme. 09/08/2016
...
//                OTMvtCaisse.setIntAMOUNT((-1) * OTMvtCaisse.getIntAMOUNT());
            ...
            new caisseManagement(this.getOdataManager(), this.getOTUser()).add_to_cash_transaction(Ojconnexion, transaction, OTMvtCaisse.getIntAMOUNT(), Description, OTTypeMvtCaisse.getStrCODECOMPTABLE(), int_AMOUNT_REMIS, int_AMOUNT_RECU, OTMvtCaisse.getLgMVTCAISSEID(), str_NUM_PIECE_COMPTABLE, Parameter.KEY_TASK_OTHER, Parameter.KEY_TASK_OTHER, reglement.getLgREGLEMENTID(), reglement.getStrFIRSTLASTNAME(), lg_MOTIF_REGLEMENT_ID, OTModeReglement.getLgTYPEREGLEMENTID().getLgTYPEREGLEMENTID(), true, bool_CHECKED, dt_CREATED);
            this.do_event_log(Ojconnexion, commonparameter.ALL, Description, this.getOTUser().getStrLOGIN(), commonparameter.statut_enable, "t_mvt_caisse", "caisse", "Mouvement de Caisse", this.getOTUser().getLgUSERID());
    ...
    public TMvtCaisse AddTMvtCaisse(TTypeMvtCaisse OTTypeMvtCaisse, String str_NUM_COMPTE, String str_NUM_PIECE_COMPTABLE, String lg_MODE_REGLEMENT_ID, double int_AMOUNT, String str_BANQUE, String str_LIEU, String str_CODE_MONNAIE, String str_COMMENTAIRE_REGLEMENT, int int_TAUX, Date dt_DATE_MVT, boolean action, String str_FIRST_LAST_NAME, String P_KEY, String str_COMMENTAIRE, int int_AMOUNT_REMIS, int int_AMOUNT_RECU, boolean bool_CHECKED, Date dt_CREATED) {
        TMvtCaisse OTMvtCaisse ;
        TModeReglement OTModeReglement ;
        ...
        OTModeReglement = (TModeReglement) this.getOdataManager().getEm().createQuery("SELECT t FROM TModeReglement t WHERE t.lgMODEREGLEMENTID LIKE ?1 OR t.strNAME LIKE ?2 OR t.strDESCRIPTION LIKE ?2")
...
//                OTMvtCaisse.setIntAMOUNT((-1) * OTMvtCaisse.getIntAMOUNT());
    ...
    public TMvtCaisse UpdateTMvtCaisse(String lg_MVT_CAISSE_ID, String lg_TYPE_MVT_CAISSE_ID, String str_NUM_COMPTE, String str_NUM_PIECE_COMPTABLE, String lg_MODE_REGLEMENT_ID, double int_AMOUNT, String str_BANQUE, String str_LIEU, String str_CODE_MONNAIE, String str_COMMENTAIRE, int int_TAUX, Date dt_DATE_MVT) {
        ...
        try {
            ...
            OTTypeMvtCaisse = (TTypeMvtCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TTypeMvtCaisse t WHERE t.lgTYPEMVTCAISSEID LIKE ?1 OR t.strNAME LIKE ?2 OR t.strDESCRIPTION LIKE ?2")
...
//        OTMvtCaisse.setStrNUMCOMPTE(str_NUM_COMPTE);
        ...
        if (OTMvtCaisse.getLgTYPEMVTCAISSEID().getLgTYPEMVTCAISSEID().equals("1")) {
            TRecettes OTRecettes = new caisseManagement(this.getOdataManager(), this.getOTUser()).AddRecette(int_AMOUNT, Parameter.KEY_TYPE_RECETTE_VENTE_BOISSON, "Entree de Caisse", OTMvtCaisse.getLgMVTCAISSEID(), int_AMOUNT, int_AMOUNT, OTMvtCaisse.getLgMVTCAISSEID(), OTModeReglement.getLgMODEREGLEMENTID(), "", bll.common.Parameter.KEY_TASK_ENTREE_CAISSE, OTReglement.getLgREGLEMENTID(), this.getOTUser().getLgUSERID(), "2", "", commonparameter.TRANSACTION_CREDIT, true);
            if (OTRecettes == null) {
                this.buildErrorTraceMessage("Impossible de valider lentree de caisse", "la recette  n'a pas pu etre MAJ");
                
                return null;
            }
            
        } else {
            TDepenses OTDepenses = new caisseManagement(this.getOdataManager(), this.getOTUser()).AddDepense(int_AMOUNT, Parameter.KEY_TYPE_DEPENSE_SORTIE_CAISSE, "Sortie de Caisse", OTMvtCaisse.getLgMVTCAISSEID(), int_AMOUNT, int_AMOUNT, OTMvtCaisse.getLgMVTCAISSEID(), OTReglement.getLgREGLEMENTID(), bll.common.Parameter.KEY_TASK_SORTIE_CAISSE, "", "", true);
            if (OTDepenses == null) {
                this.buildErrorTraceMessage("Impossible de valider la sortie de caisse", "la depense  n'a pas pu etre MAJ");
    ...
    public List<TMvtCaisse> GetMvtCaisseList(Date dtDEBUT, Date dtFin) {
        ...
        try {
            lstTMvtCaisse = this.getOdataManager().getEm().
                    createQuery("SELECT t FROM TMvtCaisse t WHERE (t.dtCREATED >= ?3 AND t.dtCREATED <= ?4) ORDER BY t.dtCREATED DESC ")
