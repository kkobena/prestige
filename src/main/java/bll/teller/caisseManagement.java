...
public class caisseManagement extends bll.bllBase {
    ...
    public TMvtCaisse OpenCaisse(String ID_COFFRE_CAISSE) {
        ...
        try {
            ...
            TCaisse oOTCaisse = new TCaisse();
            try {
                // oOTCaisse = (TCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND  t.idAnneeScolaire.idAnneeScolaire = ?2").setParameter(1, this.getOTUser().getLgUSERID()).setParameter(2, this.getOTAnneeScolaires().getIdAnneeScolaire()).getSingleResult();
...
//            oOTCaisse.setIntSOLDE(OTCoffreCaisse.getIntAMOUNT()); a decommenter en cas de probleme
            ...
            String Description = "Ouverture de la caisse de  " + this.getOTUser().getStrLOGIN() + " avec un montant de " + OTResumeCaisse.getIntSOLDEMATIN();
            this.do_event_log(Ojconnexion, commonparameter.ALL, Description, this.getOTUser().getStrLOGIN(), commonparameter.statut_enable, "t_coffre_caisse,t_resume_caisse,t_caisse", "caisse", "Mouvement de Caisse", this.getOTUser().getLgUSERID());
            ...
            /*this.setMessage(commonparameter.PROCESS_FAILED);
             this.setDetailmessage(e.getMessage());*/
    ...
    public TCoffreCaisse getTCoffreCaisseOfSomeDay(String lg_USER_ID, String str_STATUT, Date dt_Date_debut, Date dt_Date_Fin) {
        TCoffreCaisse OTCoffreCaisse = null;
        try {
            OTCoffreCaisse = (TCoffreCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TCoffreCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND  t.strSTATUT LIKE ?2  AND t.dtCREATED >= ?3  AND t.dtCREATED < ?4 ")
    ...
    public boolean sendCashToCaisseEmp(String lg_USER_ID, Integer int_AMOUNT) {
        ...
        TUser ooTuser = null;
        try {
//            TUser ooTuser = this.getOdataManager().getEm().find(TUser.class, lg_USER_ID); // a decommenter en cas de probleme
            ...
            /* String Date_debut = this.getKey().GetDateNowForSearch(0);
             ...
             new logger().OCategory.info(Date_debut + "  test   " + Date_Fin);
            ...
            /*OTCoffreCaisse = (TCoffreCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TCoffreCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND  t.strSTATUT LIKE ?2  AND t.dtCREATED >= ?3  AND t.dtCREATED < ?4 ") // a decommenter en cas de probleme 19/10/2016
             .setParameter(1, ooTuser.getLgUSERID()).
             setParameter(2, commonparameter.statut_is_Waiting_validation).
             setParameter(3, dt_Date_debut).
             setParameter(4, dt_Date_Fin).
             getSingleResult();*/
    ...
    public boolean createCoffreCaisse(TUser OTUser, double dbl_AMOUNT) {
        ...
        try {
            ...
            Ojconnexion.initConnexion();
            Ojconnexion.OpenConnexion();
            Description = "Reaprovisionement de la caisse de " + OTUser.getStrLOGIN() + " d'un montant de " + OTCoffreCaisse.getIntAMOUNT();
            this.do_event_log(Ojconnexion, commonparameter.ALL, Description, this.getOTUser().getStrLOGIN(), commonparameter.statut_enable, "t_coffre_caisse", "caisse", "Mouvement de Caisse", this.getOTUser().getLgUSERID());
            this.is_activity(Ojconnexion);
            Ojconnexion.CloseConnexion();
            this.buildSuccesTraceMessage(Description);
            result = true;
    ...
    public Double GetSoldeCaisse(TUser OTUser, Date dt_BEGIN, Date dt_END) {
        ...
        try {
            lstCaisse = this.getOdataManager().getEm().
                    createNativeQuery("CALL `proc_v_caisse`(?, ?, ?, ?, ?, ?, ?)")
                    ...
                    .setParameter(5, OTUser.getLgUSERID())
                    .setParameter(6, "%")
    ...
    public TCaisse GetTCaisse() {
        ...
        try {
            OTCaisse = (TCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TCaisse t WHERE t.lgUSERID.lgUSERID = ?1")
...
//            this.OTCaisse.setIntSOLDE(new Double(this.GetSoldeCaisse(this.getOTUser().getLgUSERID()))); // a decommenter en cas de probleme 26/03/2017
            ...
            Ojconnexion.initConnexion();
            Ojconnexion.OpenConnexion();
//            String Description = "Cloture de la caisse de  " + this.getOTUser().getStrLOGIN() + " avec un montant de " + Math.abs(OTResumeCaisse.getIntSOLDESOIR());
    ...
    public TBilletage getBilletageByResumeCaisse(String ld_CAISSE_ID) {
        ...
        try {
            OTBilletage = (TBilletage) this.getOdataManager().getEm().createQuery("SELECT t FROM TBilletage t WHERE t.ldCAISSEID = ?1")
                    .setParameter(1, ld_CAISSE_ID).getSingleResult();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return OTBilletage;
...
//                String Description = "Validation de la Cloture de la caisse de  " + this.getOTUser().getStrLOGIN() + " avec un montant de " + Math.abs(OTCaisse.getIntSOLDE() - OTResumeCaisse.getIntSOLDEMATIN());
                ...
                /*this.getOdataManager().BeginTransaction();  //ancien bon code. a decommenter en cas de probleme
                 this.getOdataManager().getEm().persist(OTResumeCaisse);
                 this.getOdataManager().getEm().persist(OTCaisse);
                 this.getOdataManager().CloseTransaction();*/
    ...
    public boolean CheckResumeCaisse(TUser ooTUser) {
        ...
        try {
            ...
            TParameters OTParameters = new TparameterManager(this.getOdataManager()).getParameter(Parameter.KEY_ACTIVATE_CLOTURE_CAISSE_AUTO);
...
//            OTResumeCaisseOld = this.getTResumeCaisseClosed(this.getOTUser().getLgUSERID());// a decommenter en cas de probleme. 19/05/2016
            OTResumeCaisseOld = this.getTResumeCaisse(this.getOTUser().getLgUSERID(), commonparameter.statut_is_Using);// a decommenter en cas de probleme. 19/05/2016
            ...
            OTResumeCaisse = (TResumeCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1  AND t.dtCREATED >= ?3  AND t.dtCREATED < ?4 AND t.strSTATUT LIKE ?5 ").
                    ...
                    .setParameter(3, dt_Date_debut)
                    .setParameter(4, dt_Date_Fin)
                    .setParameter(5, commonparameter.statut_is_Using)
                    .getSingleResult();
            if (OTResumeCaisse != null) {
                return true;
            } else {
    ...
    public int GetResumeCaisse(TUser ooTUser) {
        ...
        try {
            ...
            dt_Date_Fin = this.getKey().stringToDate(Date_Fin, this.getKey().formatterShort);
            dt_Date_debut = this.getKey().stringToDate(Date_debut, this.getKey().formatterShort);
            TResumeCaisse OTResumeCaisse = (TResumeCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1  AND t.dtCREATED >= ?3  AND t.dtCREATED < ?4 AND t.strSTATUT LIKE ?5 ").
                    setParameter(1, ooTUser.getLgUSERID()) //.setParameter(2, this.getOTAnneeScolaires().getIdAnneeScolaire())
                    .setParameter(3, dt_Date_debut).setParameter(4, dt_Date_Fin).setParameter(5, commonparameter.statut_is_Closed).getSingleResult();
...
//    public TRecettes AddRecette(Double MONTANT, String lg_TYPE_RECETTE_ID, String str_DESCRIPTION, String str_REF_FACTURE, double int_AMOUNT_REMIS, double int_AMOUNT_RECU, String str_RESSOURCE_REF, String lg_TYPE_REGLEMENT_ID, String str_type, String str_task, String lg_REGLEMENT_ID, String str_REF_COMPTE_CLIENT, String lg_MOTIF_REGLEMENT_ID, String lg_TYPE_MVT_CAISSE_ID) { // last good version. A decommenter en cas de probleme
...
//                TTypeMvtCaisse OTTypeMvtCaisse = caisseManagement.getTTypeMvtCaisse(Parameter.KEY_PARAM_MVT_VENTE_ORDONNANCE, this.getOdataManager());
                ...
                new logger().OCategory.info(" *** OTRecettes CREATION DE OTrecettes  *** " + MONTANT + " lg_TYPE_RECETTE_ID =" + lg_TYPE_RECETTE_ID + " str_type =" + str_type);
...
//                String Description = "Enregistrement d une somme de  " + OTRecettes.getIntAMOUNT() + " pour type de recette : " + OTTypeRecette.getStrTYPERECETTE();
                ...
                /*  if (!OTTypeRecette.getIsUSETVA()) {
                 str_transaction_code = commonparameter.TRANSACTION_TVA;
                 }
                 if (OTTypeRecette.getIsUSETVA()) {
                 str_transaction_code = commonparameter.TRANSACTION_CREDIT;
                 }*/
...
//                this.add_to_cash_transaction(Ojconnexion, commonparameter.TRANSACTION_CREDIT, MONTANT, Libelle, OTTypeMvtCaisse.getStrCODECOMPTABLE(), int_AMOUNT_REMIS, int_AMOUNT_RECU, str_RESSOURCE_REF, OTRecettes.getStrREFFACTURE(), str_type, str_task, lg_REGLEMENT_ID, str_REF_COMPTE_CLIENT, lg_MOTIF_REGLEMENT_ID, lg_TYPE_REGLEMENT_ID); // a decommenter en cas de probleme
                this.add_to_cash_transaction(transaction, MONTANT, Libelle, OTTypeMvtCaisse.getStrCODECOMPTABLE(), int_AMOUNT_REMIS, int_AMOUNT_RECU, str_RESSOURCE_REF, OTRecettes.getStrREFFACTURE(), str_type, str_task, lg_REGLEMENT_ID, str_REF_COMPTE_CLIENT, lg_MOTIF_REGLEMENT_ID, lg_TYPE_REGLEMENT_ID, str_TYPE, true, new Date());
                ...
                /* try {
                    ...
                    / commente le 04102017  new SnapshotManager(this.getOdataManager(), this.getOTUser()).BuildTSnapShopDalyVente(OTRecettes, (TPreenregistrement) this.find(str_REF_FACTURE, new TPreenregistrement()));
                ...
                //code ajouté
//                if(action) {
//                    new TellerMovement(this.getOdataManager(), this.getOTUser()).AddTMvtCaisse(OTTypeMvtCaisse, OTTypeMvtCaisse.getStrCODECOMPTABLE(), Libelle, "1", new Double(OTRecettes.getIntAMOUNT()), "", "", "", "", 0, new Date(), false);
//                }
                //fin code ajouté
    ...
    public void add_to_cash_transaction(jconnexion Ojconnexion, String str_TRANSACTION_REF, double int_AMOUNT, String str_DESCRIPTION, String str_NUMERO_COMPTE, double int_AMOUNT_REMIS, double int_AMOUNT_RECU, String str_RESSOURCE_REF, String str_REF_FACTURE, String str_TYPE_VENTE, String str_TASK, String lg_REGLEMENT_ID, String str_REF_COMPTE_CLIENT, String lg_MOTIF_REGLEMENT_ID, String lg_TYPE_REGLEMENT_ID, boolean str_TYPE, boolean bool_CHECKED, Date dt_CREATED) {
        ...
        try {
            ...
            /*if (OTReglement.getLgMODEREGLEMENTID().getLgMODEREGLEMENTID().equals("6") || commonparameter.TRANSACTION_DEBIT.equals(str_TRANSACTION_REF)) { // a decommenter en cas de probleme
             new logger().OCategory.info(" *** LgMODEREGLEMENTID is 6  ***  ");

             str_TRANSACTION_REF = "D";
             } else {
...
//                OTCashTransaction.setDtCREATED(new Date()); // a decommenter en cas de probleme
    ...
    //liste des coffres
    public List<TCoffreCaisse> getListeCoffreCaisse(String search_value, String ID_COFFRE_CAISSE, Date dtDEBUT, Date dtFin) {
    ...
    public List<TResumeCaisse> getListeResumeCaissesByUser(String lg_USER_ID) {
        List<TResumeCaisse> lstTResumeCaisse = new ArrayList<>();
        try {
            lstTResumeCaisse = this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1  AND  t.strSTATUT LIKE ?2 ORDER BY t.dtCREATED DESC")
                    .setParameter(1, lg_USER_ID)
                    .setParameter(2, commonparameter.statut_is_Using)
                    .getResultList();
        } catch (Exception e) {
    ...
    public TResumeCaisse getTResumeCaisse(String lg_USER_ID, String str_STATUT) {
        TResumeCaisse OTResumeCaisse = null;
        try {
            OTResumeCaisse = (TResumeCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND t.strSTATUT LIKE ?2 ORDER BY t.dtCREATED DESC")
                    .setParameter(1, lg_USER_ID)
                    .setParameter(2, str_STATUT)
    ...
    public TResumeCaisse getTResumeCaisseClosed(String lg_USER_ID) {
        TResumeCaisse OTResumeCaisse = null;
        try {
            OTResumeCaisse = (TResumeCaisse) this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID = ?1 AND (t.strSTATUT = ?2 OR t.strSTATUT = ?3) ORDER BY t.dtCREATED DESC")
                    .setParameter(1, lg_USER_ID)
    ...
    public void RollBackCloseCaisse(String ld_CAISSE_ID) {
        ...
        try {
            ...
            Ojconnexion.OpenConnexion();
//            String Description = "Cloture de la caisse de  " + this.getOTUser().getStrLOGIN() + " avec un montant de " + Math.abs(OTResumeCaisse.getIntSOLDESOIR());
            String Description = "Annulation de la clôture de la caisse de " + OTResumeCaisse.getLgUSERID().getStrFIRSTNAME() + " " + OTResumeCaisse.getLgUSERID().getStrLASTNAME() + " par " + this.getOTUser().getStrFIRSTNAME() + " " + this.getOTUser().getStrLASTNAME() + " effectuée avec succès";
            this.do_event_log(Ojconnexion, commonparameter.ALL, Description, this.getOTUser().getStrLOGIN(), commonparameter.statut_enable, "t_coffre_caisse,t_resume_caisse,t_caisse", "caisse", "Mouvement de Caisse", this.getOTUser().getLgUSERID());
            this.is_activity(Ojconnexion);
            Ojconnexion.CloseConnexion();
            this.setMessage(commonparameter.PROCESS_SUCCESS);
    ...
    public void sendNotification(String lg_USER_ID) {
        ...
        try {
            ...
            List<TResumeCaisse> lstTResumeCaisse = this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND (t.dtUPDATED >= ?3 AND t.dtUPDATED <= ?4) AND t.lgUSERID.lgEMPLACEMENTID.lgEMPLACEMENTID LIKE ?5 ORDER BY t.dtCREATED DESC")
    ...
    public void sendSmsChiffreAffaireByCaisse(Date dt_Date_Debut, Date dt_Date_Fin, String lg_USER_ID) {
        ...
        try {
            ...
            lstTResumeCaisse = this.getOdataManager().getEm().createQuery("SELECT t FROM TResumeCaisse t WHERE t.lgUSERID.lgUSERID LIKE ?1 AND (t.dtUPDATED >= ?3 AND t.dtUPDATED <= ?4) AND t.lgUSERID.lgEMPLACEMENTID.lgEMPLACEMENTID LIKE ?5 ORDER BY t.dtCREATED DESC")
                    .setParameter(1, lg_USER_ID).setParameter(3, dt_Date_Debut).setParameter(4, dt_Date_Fin).setParameter(5, lg_EMPLACEMENT_ID).getResultList();
            for (TResumeCaisse OTResumeCaisse : lstTResumeCaisse) {
                totalAmount += OTResumeCaisse.getIntSOLDESOIR();
                message += "- " + OTResumeCaisse.getLgUSERID().getStrFIRSTNAME() + " " + OTResumeCaisse.getLgUSERID().getStrLASTNAME() + ": " + conversion.AmountFormat(OTResumeCaisse.getIntSOLDESOIR(), '.') + "\n";
            ...
            message += "Total Caisse: " + conversion.AmountFormat(totalAmount.intValue(), '.');
            if (lstTResumeCaisse.size() > 0) {
                lstTAlertEventUserFone = this.getOdataManager().getEm().createQuery("SELECT t FROM TAlertEventUserFone t WHERE t.strEvent.strEvent LIKE ?1")
                        .setParameter(1, "N_GET_SOLDE_CAISSE").getResultList();
                for (TAlertEventUserFone OTAlertEventUserFone : lstTAlertEventUserFone) {
//                    OServicesNotifCustomer.doservice(message, OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE(), this.getKey().getShortId(10)+OTAlertEventUserFone.getLgUSERFONEID().getLgUSERID().getLgUSERID());
                    OServicesNotifCustomer.saveNotification(message, OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE(), this.getKey().getShortId(10) + OTAlertEventUserFone.getLgUSERFONEID().getLgUSERID().getLgUSERID());
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    //sms de mi-journé
    public void sendSmsChiffreAffaireMiDay(String dt_date_debut, String dt_date_fin, String h_debut, String h_fin, String lg_USER_ID, String lg_TYPE_REGLEMENT_ID) {
        ...
        try {
            ...
            listPreenregistrement = OJournalVente.getListeCaisse(dt_date_debut, dt_date_fin, h_debut, h_fin, lg_USER_ID, lg_TYPE_REGLEMENT_ID);
            ...
            if (int_PRICE_TOTAL > 0) {
                lstTAlertEventUserFone = this.getOdataManager().getEm().createQuery("SELECT t FROM TAlertEventUserFone t WHERE t.strEvent.strEvent LIKE ?1")
                        .setParameter(1, "N_GET_SOLDE_CAISSE").getResultList();
                for (TAlertEventUserFone OTAlertEventUserFone : lstTAlertEventUserFone) {
//                    if(OServicesNotifCustomer.doservice(message, OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE(), dt_date_debut+OTAlertEventUserFone.getLgUSERFONEID().getLgUSERID().getLgUSERID()) > 0) { //a decommenter en cas de probleme 09/08/2016
                    if (OServicesNotifCustomer.saveNotification(message, OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE(), dt_date_debut + OTAlertEventUserFone.getLgUSERFONEID().getLgUSERID().getLgUSERID()) != null) {
