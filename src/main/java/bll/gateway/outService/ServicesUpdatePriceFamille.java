...
public class ServicesUpdatePriceFamille extends bll.bllBase implements Iservice {
    ...
    public String doservice(TAlertEventUserFone OTAlertEventUserFone) {
        ...
        try {
            List<TEventLog> LstTEventLog = new EventLogManagement(this.getOdataManager()).GetLstTEventLog(Parameter.KEY_TABLE_TFAMILLE);
            this.getOdataManager().BeginTransaction();
            for (int i = 0; i < LstTEventLog.size(); i++) {
                this.refresh(LstTEventLog.get(i));
                new logger().OCategory.info(LstTEventLog.get(i).getStrDESCRIPTION() + "****" + OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE());
                this.buildSuccesTraceMessage("result : " + str_result + "   ");
                // this.BuidlDataToNotify(LstTEventLog.get(i).getStrDESCRIPTION() + "", OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE()); //a decommenter en cas de probleme 09/08/2016
                 this.saveNotification(LstTEventLog.get(i).getStrDESCRIPTION() + "", OTAlertEventUserFone.getLgUSERFONEID().getStrPHONE(), this.getKey().getComplexId()); //
    ...
    public String BuidlDataToNotify(String str_result, String str_phone) {
        ...
        try {
           ...
           /* TOutboudMessage OTOutboudMessage = new TOutboudMessage();
            ...
            TOutboudMessage OTOutboudMessage = this.saveNotification(str_result, str_phone, this.getKey().getComplexId());
...
//            }
    ...
    public List<TOutboudMessage> getListeMessageWaiting(String search_value, String lg_OUTBOUND_MESSAGE_ID, int int_MAX_ROW) {
        ...
        try {
            ...
            if(int_MAX_ROW == 0) {
                  lstTOutboudMessage = this.getOdataManager().getEm().createQuery("SELECT t FROM TOutboudMessage t WHERE t.strPHONE LIKE ?1 AND t.strSTATUT = ?2 AND t.lgOUTBOUNDMESSAGEID LIKE ?3")
                    .setParameter(1, search_value + "%").setParameter(2, commonparameter.statut_is_Waiting).setParameter(3, lg_OUTBOUND_MESSAGE_ID).getResultList();
            } else {
                  lstTOutboudMessage = this.getOdataManager().getEm().createQuery("SELECT t FROM TOutboudMessage t WHERE t.strPHONE LIKE ?1 AND t.strSTATUT = ?2 AND t.lgOUTBOUNDMESSAGEID LIKE ?3")
                    .setParameter(1, search_value + "%").setParameter(2, commonparameter.statut_is_Waiting).setParameter(3, lg_OUTBOUND_MESSAGE_ID).setMaxResults(int_MAX_ROW).getResultList();
    ...
    public TOutboudMessage getTOutboudMessage(String str_REF) {
        TOutboudMessage OTOutboudMessage = null;
        try {
            OTOutboudMessage = (TOutboudMessage) this.getOdataManager().getEm().createQuery("SELECT t FROM TOutboudMessage t WHERE t.strREF = ?1")
