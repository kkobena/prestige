...
public class ServicesNotifCustomer extends bll.bllBase implements Iservice {
    ...
    public String BuidlDataToNotify(String str_result, String str_phone, String str_REF) {
        ...
        try {
            /*OTOutboudMessage = this.getTOutboudMessage(str_REF); //a decommenter en cas de probleme 09/08/2016
...
//            }           
    ...
    /*@Override
     ...
     int i = 0;
     try {
     datas = OPreenregistrement.generationDataForNotif();
     new logger().OCategory.info("datas taille " + datas.size());
     for (String OString : datas) {
     new logger().OCategory.info("OString " + OString);
     String[] tabString = OString.split("|");
     this.BuidlDataToNotify(tabString[0], tabString[1]);
     ...
     this.buildErrorTraceMessage(i + "/" + datas.size() + " messages ont été envoyé");
     }
     }
