...
public class ServiceStock extends bll.bllBase implements Iservice {
    ...
    public String doservice(TInboudMessage OTInboudMessage) {
        ...
        try {
            /*
             * List<TProductItemStock> LstTProductItemStock = new ArrayList<TProductItemStock>(); LstTProductItemStock =
             * this.getOdataManager().getEm().
             * createQuery("SELECT t FROM TProductItemStock t ORDER BY t.lgPRODUCTITEMID.strNAME,t.dtCREATED  ").
             * getResultList(); for (int i = 0; i < LstTProductItemStock.size(); i++) { TProductItemStock
             * OTProductItemStock = LstTProductItemStock.get(i); str_result =
             * OTProductItemStock.getLgPRODUCTITEMID().getStrNAME() + "(" + OTProductItemStock.getIntNUMBER() + ");" +
             * str_result; }
             */
            str_result = "Un test";
