-- Privilege de cloture d'inventaire (bouton 'Cloturer' de la fiche d'inventaire).
-- Controle applique cote ecran (bouton masque) ET cote serveur (endpoint de cloture).
INSERT IGNORE INTO t_privilege (`lg_PRIVELEGE_ID`, `str_NAME`, `str_TYPE`, `str_DESCRIPTION`, `lg_PRIVELEGE_ID_DEP`, `dt_CREATED`, `lg_CREATED_BY`, `dt_UPDATED`, `lg_UPDATED_BY`, `str_STATUT`)
	VALUES ('20260718', 'P_CLOTURER_INVENTAIRE', 'CUSTOMER', 'Autorisation de cloturer un inventaire', NULL, NOW(), NULL, NOW(), NULL, 'enable');
...
SELECT LEFT(UUID(), 40), r.lg_ROLE_ID, '20260718', NOW(), NOW()
FROM t_role r
WHERE r.lg_ROLE_ID NOT IN (SELECT rp.lg_ROLE_ID FROM t_role_privelege rp WHERE rp.lg_PRIVILEGE_ID = '20260718');
