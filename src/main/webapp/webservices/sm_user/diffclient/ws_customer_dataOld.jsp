...
<%    String lg_COMPTE_CLIENT_ID = "%%", search_value = "", str_CODE_INTERNE = "%%", str_FIRST_NAME = "", lg_TYPE_CLIENT_ID = "%%", str_LAST_NAME = "", lg_CLIENT_ID = "%%";
    ...
    if (lg_TYPE_CLIENT_ID != null && lg_TYPE_CLIENT_ID != "") {
        ...
        total = OclientManagement.showOnorAllClient(search_value).size();
    }
...
<%    int virtual_liste_size = 0;
        ...
        for (int i = 0; i < (total < limit ? total : limit); i++) {
            ...
            if (lstTClient.get(i).getLgTYPECLIENTID().getLgTYPECLIENTID().equalsIgnoreCase("1")) {
                ...
                if (OTAyantDroit != null) {
                    ...
                    lg_RISQUE_ID = OTAyantDroit.getLgRISQUEID().getLgRISQUEID();
                ...
                for (int k = 0; k < lstTCompteClientTiersPayant.size(); k++) {
                   ...
                   // json.put("COMPTCLTTIERSPAYANT", cmpta);
                    ...
                    /* a decommenter en cas de probleme
                     if (lstTCompteClientTiersPayant.get(k).getBISRO() != null || lstTCompteClientTiersPayant.get(k).getBISRC1() != null || lstTCompteClientTiersPayant.get(k).getBISRC2() != null) {
                     if (lstTCompteClientTiersPayant.get(k).getBISRO().booleanValue() == true) {
                     //json.put("RO", lstTCompteClientTiersPayant.get(k).getLgTIERSPAYANTID().getStrCODEORGANISME());
                     json.put("RO", lstTCompteClientTiersPayant.get(k).getLgTIERSPAYANTID().getStrNAME());
                     json.put("RO_TAUX", lstTCompteClientTiersPayant.get(k).getIntPOURCENTAGE());
                     json.put("lg_COMPTE_CLIENT_TIERS_PAYANT_ID", lstTCompteClientTiersPayant.get(k).getLgCOMPTECLIENTTIERSPAYANTID());
                     } else if (lstTCompteClientTiersPayant.get(k).getBISRC1().booleanValue() == true) {
                     json.put("RC1", lstTCompteClientTiersPayant.get(k).getLgTIERSPAYANTID().getStrNAME());
                     json.put("RC1_TAUX", lstTCompteClientTiersPayant.get(k).getIntPOURCENTAGE());
                     json.put("lg_COMPTE_CLIENT_TIERS_PAYANT_ID", lstTCompteClientTiersPayant.get(k).getLgCOMPTECLIENTTIERSPAYANTID());
                     } else if (lstTCompteClientTiersPayant.get(k).getBISRC2().booleanValue() == true) {
                     json.put("RC2", lstTCompteClientTiersPayant.get(k).getLgTIERSPAYANTID().getStrNAME());
                     json.put("RC2_TAUX", lstTCompteClientTiersPayant.get(k).getIntPOURCENTAGE());
                     json.put("lg_COMPTE_CLIENT_TIERS_PAYANT_ID", lstTCompteClientTiersPayant.get(k).getLgCOMPTECLIENTTIERSPAYANTID());
                     }
                     } else {
                     json.put("RC1", "");
                 ...
                 cmpta.put(cmpt);
                 json.put("COMPTCLTTIERSPAYANT", cmpta);
                    System.out.println("cmpta " + cmpta);
                arrayObj.put(json);
    ...
    new logger().OCategory.info("result ws_customer_data   ----  " + result);
%>
