<%-- 
    Document   : ws_data_details_vente
    Created on : 8 déc. 2015, 16:08:52
    Author     : KKOFFI
--%>
...
String lg_PREENREGISTREMENT_ID = "%%";
  if (request.getParameter("str_STATUT") != null) {
    ...
    lstTPreenregistrementDetail = OdataManager.getEm().createQuery("SELECT t FROM TPreenregistrementDetail t WHERE t.lgPREENREGISTREMENTID.lgPREENREGISTREMENTID = ?1 ORDER BY t.dtCREATED DESC")
   ...
   // new logger().OCategory.info("str_STATUT  " + str_STATUT);
    ...
    for (int i = 0; i < lstTPreenregistrementDetail.size(); i++) {
       ...
       // json.put("int_FAMILLE_PRICE", lstTPreenregistrementDetail.get(i).getLgFAMILLEID().getIntPRICE()); ancien bon code
        ...
        json.put("dt_CREATED", date.backabaseUiFormat.format(lstTPreenregistrementDetail.get(i).getDtCREATED()));
