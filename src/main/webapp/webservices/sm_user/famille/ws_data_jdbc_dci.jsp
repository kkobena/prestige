...
<%    new logger().OCategory.info("dans ws data famille jdbc dci ");
    ...
    if (str_TYPE_TRANSACTION.equalsIgnoreCase("%%") || str_TYPE_TRANSACTION.equalsIgnoreCase("ALL")) {
        lstTFamille = OfamilleManagement.getListArticleDCIByJdbc(search_value, lg_FAMILLE_ID, start, limit);
        total = OfamilleManagement.getListArticleDCIByJdbc(search_value, lg_FAMILLE_ID).size();
    } else if (str_TYPE_TRANSACTION.equalsIgnoreCase("init")) {
        lstTFamille = OfamilleManagement.getListArticleDCIByJdbcInit(search_value, lg_FAMILLE_ID, start, limit);
        total = OfamilleManagement.getListArticleDCIByJdbcInit(search_value, lg_FAMILLE_ID).size();
    } else {
...
<%    for (int i = 0; i < (lstTFamille.size() < limit ? lstTFamille.size() : limit); i++) {
        ...
        json.put("CIP", lstTFamille.get(i).getIntCIP());json.put("str_DESCRIPTION_PLUS", lstTFamille.get(i).getIntCIP() + " " + lstTFamille.get(i).getStrDESCRIPTION() + " (" + lstTFamille.get(i).getIntPRICE() + ")");
        json.put("int_NUMBER_AVAILABLE", OTFamilleStock.getIntNUMBERAVAILABLE());
        json.put("int_NUMBER", OTFamilleStock.getIntNUMBER());
        json.put("lg_ZONE_GEO_ID", OTFamilleStock.getLgFAMILLEID().getStrCODETABLEAU());
