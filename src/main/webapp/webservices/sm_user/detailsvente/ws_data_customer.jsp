...
<%
    String lg_PREENREGISTREMENT_ID = "%%", lg_PREENREGISTREMENT_DETAIL_ID = "%%", str_REF = "", lg_FAMILLE_ID = "%%", dt_CREATED = "%%";
    ...
    if (request.getParameter("lg_PREENREGISTREMENT_DETAIL_ID") != null) {
        if (request.getParameter("lg_PREENREGISTREMENT_DETAIL_ID").toString().equals("ALL")) {
    ...
    if (request.getParameter("lg_PREENREGISTREMENT_ID") != null) {
        if (request.getParameter("lg_PREENREGISTREMENT_ID").toString().equals("ALL")) {
    ...
    try {
         OTPreenregistrement = OPreenregistrement.getTPreenregistrementByRef(str_REF);
        if (OTPreenregistrement != null) {
            ...
            TPreenregistrementCompteClient OTPreenregistrementCompteClient = OPreenregistrement.getTPreenregistrementCompteClient(OTPreenregistrement.getLgPREENREGISTREMENTID());
            str_FIRST_LAST_NAME = OTPreenregistrementCompteClient.getLgCOMPTECLIENTID().getLgCLIENTID().getStrFIRSTNAME() + " " + OTPreenregistrementCompteClient.getLgCOMPTECLIENTID().getLgCLIENTID().getStrFIRSTNAME();
    ...
    lstTPreenregistrementDetail = OdataManager.getEm().createQuery("SELECT t FROM TPreenregistrementDetail t WHERE t.lgPREENREGISTREMENTID.lgPREENREGISTREMENTID LIKE ?1 AND t.lgFAMILLEID.strNAME LIKE ?2 AND t.strSTATUT LIKE ?3 ORDER BY t.dtCREATED DESC")
            ...
            .getResultList();
    new logger().OCategory.info("lstTPreenregistrementDetail size " + lstTPreenregistrementDetail.size());
    new logger().OCategory.info("str_STATUT  " + str_STATUT);
    ...
    for (int i = 0; i < lstTPreenregistrementDetail.size(); i++) {
        ...
        json.put("int_FAMILLE_PRICE", lstTPreenregistrementDetail.get(i).getLgFAMILLEID().getIntPRICE());
        ...
        json.put("dt_CREATED", date.DateToString(lstTPreenregistrementDetail.get(i).getDtCREATED(), key.formatterMysql));
        ...
        json.put("lg_TYPE_VENTE_ID", lstTPreenregistrementDetail.get(i).getLgPREENREGISTREMENTID().getLgTYPEVENTEID().getLgTYPEVENTEID());

        str_MEDECIN = lstTPreenregistrementDetail.get(i).getLgPREENREGISTREMENTID().getStrMEDECIN();
        lg_TYPE_VENTE_ID = lstTPreenregistrementDetail.get(i).getLgPREENREGISTREMENTID().getLgTYPEVENTEID().getStrNAME();
    ...
    String result = "({\"total\":\"" + lstTPreenregistrementDetail.size() + "\",\"results\":" + arrayObj.toString() + ",\"errors_code\": \"" + code_statut + "\", \"errors\": \"" + desc_statut + "\", \"str_MEDECIN\":\"" + str_MEDECIN + "\", \"str_FIRST_LAST_NAME\":\"" + str_FIRST_LAST_NAME + "\",\"lg_TYPE_VENTE_ID\":\"" + lg_TYPE_VENTE_ID + "\",\"dbl_total_remise\":\"" + dbl_total_remise + " \",\"total_vente\":\"" + int_total_vente + "\",\"int_total_product\": \"" + int_total_product + "\"})";
