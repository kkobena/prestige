...
<%
    ...
    if (request.getParameter("mode") != null) {
        ...
        if (request.getParameter("mode").toString().equals("assort")) {

            OStockManager.doReassort(int_NUMBER, lg_FAMILLE_ID, commonparameter.ASSORT);
            ObllBase.setDetailmessage(OStockManager.getDetailmessage());
            ObllBase.setMessage(OStockManager.getMessage());
        } else if (request.getParameter("mode").toString().equals("reassort")) {

            OStockManager.doReassort(int_NUMBER, lg_FAMILLE_ID, commonparameter.REASSORT);
            ObllBase.setDetailmessage(OStockManager.getDetailmessage());
            ObllBase.setMessage(OStockManager.getMessage());

        } else if (request.getParameter("mode").toString().equals("destockage")) {

            OStockManager.doDestockage(int_NUMBER, lg_FAMILLE_ID);
            ObllBase.setDetailmessage(OStockManager.getDetailmessage());
            ObllBase.setMessage(OStockManager.getMessage());
