...
<%
    String lg_RETOURDEPOT_ID = "", lg_IMPRIMANTE_ID = "2";
    ...
    String scr_report_file = "rp_retourdepot";
    ...
    OreportManager.setPath_report_pdf(Ojdom.scr_report_pdf + "retourdepot_" + report_generate_file);

    bllBase obllBase = new bllBase();
    obllBase.checkDatamanager();
    TRetourdepot oTRetourdepot = obllBase.getOdataManager().getEm().find(TRetourdepot.class, lg_RETOURDEPOT_ID);
    TOfficine oTOfficine = obllBase.getOdataManager().getEm().find(dal.TOfficine.class, "1");
     TEmplacement emplacement=null;  
    // Récupération des paramètres
    // Entete
    try {
          emplacement=obllBase.getOdataManager().getEm().find(dal.TEmplacement.class, oTRetourdepot.getPkey());
  
        } catch (Exception e) {
        }
    
    String P_H_CLT_INFOS = "EDITION RETOUR DEPÔT "+(emplacement!=null?emplacement.getStrNAME():"");
...
parameters.put("P_AUTRE_DESC",  oTOfficine.getStrFIRSTNAME() + " " + oTOfficine.getStrLASTNAME());
   String finalphonestring = oTOfficine.getStrPHONE() != null ? "Tel: " + conversion.PhoneNumberFormat("+225", oTOfficine.getStrPHONE()) : "";
    ...
    parameters.put("P_REFERENCE", lg_RETOURDEPOT_ID);
    parameters.put("P_H_lg_ORDER_ID", oTRetourdepot.getStrNAME());
    parameters.put("P_H_lg_GROSSISTE_ID", oTOfficine.getStrNOMCOMPLET());
    ...
    response.sendRedirect("../../../data/reports/pdf/" + "retourdepot_" + report_generate_file);
