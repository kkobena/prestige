...
<%! Translate oTranslate = new Translate();
    ...
    List<dal.TEtiquette> lstTEtiquette = new ArrayList<dal.TEtiquette>();
%>
...
<%
    ...
    if (request.getParameter("lg_ETIQUETTE_ID") != null) {
        if (request.getParameter("lg_ETIQUETTE_ID").toString().equals("ALL")) {
            lg_ETIQUETTE_ID = "%%";
        } else {
            lg_ETIQUETTE_ID = request.getParameter("lg_ETIQUETTE_ID").toString();
        }
    ...
    lstTEtiquette = OdataManager.getEm().createQuery("SELECT t FROM TEtiquette t WHERE t.lgETIQUETTEID LIKE ?1 AND t.strNAME LIKE ?2 AND t.strSTATUT LIKE ?3 ").
            setParameter(1, lg_ETIQUETTE_ID)
            .setParameter(2, Os_Search_poste.getOvalue())
            .setParameter(3, commonparameter.statut_enable)
            .getResultList();
    new logger().OCategory.info(lstTEtiquette.size());
%>   
...
<%    JSONArray arrayObj = new JSONArray();
    for (int i = pgInt; i < pgInt_Last; i++) {
        ...
        json.put("str_STATUT", lstTEtiquette.get(i).getStrSTATUT());

        dt_CREATED = lstTEtiquette.get(i).getDtCREATED();
        if (dt_CREATED != null) {
            json.put("dt_CREATED", key.DateToString(dt_CREATED, key.formatterOrange));
        }

        dt_UPDATED = lstTEtiquette.get(i).getDtUPDATED();
        if (dt_UPDATED != null) {
