...
<html>
    ...
    <%
        ...
        String dt_end = date.formatterMysqlShort.format(new Date()), search_value = "%%";
        ...
        String scr_report_file = "rp_statisticventereglsociete_";
        String report_generate_file = key.GetNumberRandom();
        report_generate_file = report_generate_file + ".pdf";
        OreportManager.setPath_report_src(Ojdom.scr_report_file + scr_report_file + ".jrxml");
        OreportManager.setPath_report_pdf(Ojdom.scr_report_pdf + "rp_statisticventereglsociete_" + report_generate_file);

        String P_H_INSTITUTION = oTOfficine.getStrNOMABREGE();
        String P_INSTITUTION_ADRESSE = oTOfficine.getStrADRESSSEPOSTALE();
        String periode = date.FORMATTERYEAR.format(java.sql.Date.valueOf(dt_start));
        String P_PERIODE = "de " + periode;
        if (!"".equals(dt_end) && !date.FORMATTERYEAR.format(java.sql.Date.valueOf(dt_end)).equals(date.FORMATTERYEAR.format(java.sql.Date.valueOf(dt_start)))) {
            P_PERIODE = "de " + periode + " à " + date.FORMATTERYEAR.format(java.sql.Date.valueOf(dt_end));
            parameters.put("P_DATE_END", dt_end );
        } else {
            if("".equals(dt_end)){
                parameters.put("P_DATE_END", dt_start );
           
            }else {parameters.put("P_DATE_END", dt_end );}
            
        }
        parameters.put("P_DATE_START", dt_start);
        parameters.put("P_H_CLT_INFOS", "Evolution mensuelle des ventes \net des encaissements selon société " + P_PERIODE);
        ...
        response.sendRedirect("../../../data/reports/pdf/" + "rp_statisticventereglsociete_" + report_generate_file);
