var url_services_data_ventemanagerbis = '../webservices/sm_user/vente/ws_vente_without_bon.jsp';
var url_services_data_ventemanagerbis_generate_pdf = '../webservices/sm_user/vente/ws_generate_vente_withoutbon_pdf.jsp';
...
Ext.define('testextjs.view.sm_user.vente.VenteManagerBis', {
    extend: 'Ext.grid.Panel',
    xtype: 'ventemanagerbis',
    id: 'ventemanagerbisID',
    frame: true,
    animCollapse: false,
    title: 'Liste des ventes sans numero de bon',
    plain: true,
    maximizable: true,
//    tools: [{type: "pin"}],
    closable: false,
//    iconCls: 'icon-grid',
    ...
    initComponent: function() {
        ...
        var store = new Ext.data.Store({
            ...
            proxy: {
                type: 'ajax',
                url: url_services_data_ventemanagerbis,
                reader: {
                    type: 'json',
                    root: 'results',
                    totalProperty: 'total'
            ...
            columns: [{
                ...
                },
                {
                    ...
                    items: [{
                            ...
                            tooltip: 'Mettre a jour le numero de bon',
                            scope: this,
                ...
                },
                {
                    ...
                    items: [{
                            ...
                            tooltip: 'Cl&ocirc;turer la mise &agrave; jour le numero de bon',
                            scope: this,
                            handler: this.onCloseUpdateBonClick,
                            getClass: function(value, metadata, record) {
                                if (record.get('BTN_ANNULATION') === 'false') {
                                    return 'x-hide-display';
                                } else {
                    ...
                    listeners: {
                        'change': function(me) {
                            ...
                            /*dt_Date_Debut = me.getSubmitValue();
                             Ext.getCmp('Grid_Prevente_ID').getStore().getProxy().url = url_services_data_ventemanagerbis + "?&dt_Date_Debut=" + dt_Date_Debut;
                             */
                        }
                    }
                }, {
                    ...
                    listeners: {
                        'change': function(me) {
                            ...
                            /*  dt_Date_Fin = me.getSubmitValue();
                             Ext.getCmp('Grid_Prevente_ID').getStore().getProxy().url = url_services_data_ventemanagerbis + "?&dt_Date_Debut=" + dt_Date_Debut + "&dt_Date_Fin=" + dt_Date_Fin;
                             */
                        }
                    }
                }, {
                    ...
                    listeners: {
                        'change': function(me) {
                            ...
                            /* h_debut = me.getSubmitValue();
                             Ext.getCmp('Grid_Prevente_ID').getStore().getProxy().url = url_services_data_ventemanagerbis + "?&dt_Date_Debut=" + dt_Date_Debut + "&dt_Date_Fin=" + dt_Date_Fin + "&h_debut=" + h_debut;
                             */
                        }
                    }
                }, {
                    ...
                    listeners: {
                        'change': function(me) {
                            ...
                            /* h_fin = me.getSubmitValue();
                             Ext.getCmp('Grid_Prevente_ID').getStore().getProxy().url = url_services_data_ventemanagerbis + "?&dt_Date_Debut=" + dt_Date_Debut + "&dt_Date_Fin=" + dt_Date_Fin + "&h_debut=" + h_debut + "&h_fin=" + h_fin;
                             */
                        }
                    }
                },
...
//                    hidden: true, //a retirer demain
    ...
    onPdfClick: function(grid, rowIndex) {
        ...
        var linkUrl = url_services_data_ventemanagerbis_generate_pdf + '?dt_Date_Debut=' + dt_Date_Debut_search_val + '&dt_Date_Fin=' + dt_Date_Fin_search_val + '&h_debut=' + h_debut_search_val + '&h_fin=' + h_fin_search_val;
    ...
    onPdfListVenteClick: function() {
        ...
        var linkUrl = url_services_data_ventemanagerbis_generate_pdf + '?dt_Date_Debut=' + Ext.getCmp('dt_debut_journal').getSubmitValue() + "&dt_Date_Fin=" + Ext.getCmp('dt_fin_journal').getSubmitValue() + "&search_value=" + search_value + "&lg_TIERS_PAYANT_ID=" + lg_TIERS_PAYANT_ID + "&h_debut=" + h_debut + "&h_fin=" + h_fin + "&title=RELEVE DES VENTES A CREDIT";
...
//alert('linkUrl'+linkUrl);
    ...
    onEditClick: function(grid, rowIndex) {
        var rec = grid.getStore().getAt(rowIndex);
        new testextjs.view.sm_user.vente.action.add({
            odatasource: rec.data,
            parentview: this,
            mode: "update",
            titre: "Mise a jour du bon de la vente  [" + rec.get('str_REF') + "]"
        });
    },
    onRechClick: function() {
        ...
        }, url_services_data_ventemanagerbis);
    },
    onCloseUpdateBonClick: function(grid, rowIndex) {
        var rec = grid.getStore().getAt(rowIndex);
        Ext.MessageBox.confirm('Message',
                'Voulez-vous cl&ocirc;turer la mise &agrave; jour des bons de la vente ' + rec.get('str_REF'),
                function(btn) {
                    if (btn === 'yes') {

                        Ext.Ajax.request({
                            timeout: 240000,
                            url: url_services_transaction_detailsvente_ventecloturee + 'closurebon',
                            params: {
